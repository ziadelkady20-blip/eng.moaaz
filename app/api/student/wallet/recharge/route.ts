...
export async function POST(req:Request){
  try{
    ...
    const senderPhone=String(form.get('senderPhone')||'').trim()||null
    ...
    if(senderPhone&&!/^01\d{9}$/.test(senderPhone)) return NextResponse.json({error:'رقم الهاتف غير صحيح'},{status:400})
    ...
    const duplicate=reference?await db.$queryRaw<any[]>(Prisma.sql`SELECT "id" FROM "RechargeRequest" WHERE "walletId"=${walletId} AND "reference"=${reference} LIMIT 1`):[]
    if(duplicate.length) return NextResponse.json({error:'رقم العملية مستخدم بالفعل'},{status:409})
    const id=randomUUID()
    await db.$executeRaw(Prisma.sql`INSERT INTO "RechargeRequest" ("id","walletId","amount","method","senderPhone","reference","proofData","proofMime") VALUES (${id},${walletId},${amount},${method},${senderPhone},${reference},${proofData},${proof.type})`)
