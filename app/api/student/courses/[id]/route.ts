...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!u.student.gradeId) return NextResponse.json({ error: 'حساب الطالب غير مرتبط بصف دراسي' }, { status: 400 })
    const course = await db.course.findFirst({
      where: { id, published: true, gradeId: u.student.gradeId },
      include: { grade: true, subject: true, teacher: { include: { user: true } }, modules: { orderBy: { order: 'asc' }, include: { lessons: { orderBy: { order: 'asc' }, include: { video: true } } } } },
    })
    ...
    const completedLessons = course.modules.reduce((n, m) => n + m.lessons.filter(l => map.get(l.id)?.completed).length, 0)
    return NextResponse.json({ course: {
      id: course.id, title: course.title, description: course.description, coverUrl: course.coverUrl,
      price: Number(course.price), grade: course.grade.name, subject: course.subject.name, teacher: course.teacher.user.name,
      enrolled, balance, totalLessons, completedLessons, progress: totalLessons ? Math.round(completedLessons / totalLessons * 100) : 0,
      modules: course.modules.map(m => ({ id: m.id, title: m.title, order: m.order, lessons: m.lessons.map(l => ({
        id: l.id, title: l.title, order: l.order, hasVideo: !!l.video, locked: !enrolled,
        progress: map.get(l.id)?.watchedPct ?? 0, completed: map.get(l.id)?.completed ?? false,
      })) })),
