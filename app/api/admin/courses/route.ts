import {NextResponse} from 'next/server'; import {db} from '@/lib/db'; import {Prisma} from '@prisma/client'; import {requireRole} from '@/lib/auth'; import {z} from 'zod'
...
export async function POST(req:Request){try{await requireRole(['ADMIN']); const data=schema.parse(await req.json()); const [subject,teacher]=await Promise.all([db.subject.findFirst({orderBy:{name:'asc'}}),db.teacher.findFirst({orderBy:{user:{name:'asc'}}})]); if(!subject||!teacher)return NextResponse.json({error:'لا توجد مادة أو مدرس مضبوطين للمنصة'},{status:400}); const course=await db.course.create({data:{title:data.title,description:data.description||null,price:data.price,gradeId:data.gradeId,published:data.published??false,subjectId:subject.id,teacherId:teacher.id}}); return NextResponse.json({course},{status:201})}catch(e){return NextResponse.json({error:e instanceof z.ZodError?'بيانات الكورس غير مكتملة':'تعذر إنشاء الكورس'},{status:400})}}
