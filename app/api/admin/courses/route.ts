  ...
  try{
    ...
    const courses=await db.course.findMany({
      include:{
        ...
        modules:{include:{lessons:{include:{video:true}}},orderBy:{order:'asc'}},
...
export async function POST(req:Request){
  try{
    ...
    const [subject,teacher]=await Promise.all([
      db.subject.findFirst({orderBy:{name:'asc'}}),
      db.teacher.findFirst({orderBy:{user:{name:'asc'}}})
    ])
    if(!subject||!teacher){
    ...
    return NextResponse.json({
      error:e instanceof z.ZodError?'بيانات الكورس غير مكتملة':'تعذر إنشاء الكورس'
    },{status:400})
  }
}
...
export async function PATCH(req:Request){
  try{
    ...
    return NextResponse.json({
      error:e instanceof z.ZodError?'بيانات التعديل غير صحيحة':'تعذر تعديل الكورس'
    },{status:400})
  }
}
