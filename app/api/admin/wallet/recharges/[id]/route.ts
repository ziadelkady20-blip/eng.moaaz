...
export async function POST(req:Request,{params}:{params:Promise<{id:string}>}){
  try{
    const admin=await requireAdminAccess(); const {id}=await params; const body=await req.json().catch(()=>null); const action=body?.action; const note=typeof body?.note==='string'?body.note.trim().slice(0,500):null
    if(action!=='APPROVE'&&action!=='REJECT') return NextResponse.json({error:'إجراء غير صحيح'},{status:400})
    const result=await db.$transaction(async tx=>{
      const rows=await tx.$queryRaw<any[]>(Prisma.sql`SELECT r."id",r."walletId",r."amount",r."status",w."balance",s."userId" FROM "RechargeRequest" r JOIN "Wallet" w ON w."id"=r."walletId" JOIN "Student" s ON s."id"=w."studentId" WHERE r."id"=${id} FOR UPDATE`)
      const r=rows[0]; if(!r) throw new Error('طلب الشحن غير موجود'); if(r.status!=='PENDING') throw new Error('تمت مراجعة هذا الطلب بالفعل')
      if(action==='REJECT'){
        ...
        await tx.$executeRaw(Prisma.sql`INSERT INTO "Notification" ("id","userId","title","body") VALUES (${randomUUID()},${r.userId},'تم رفض طلب الشحن',${note?`تم رفض طلب شحن المحفظة. ${note}`:'تم رفض طلب شحن المحفظة بعد المراجعة.'})`)
      ...
      await tx.$executeRaw(Prisma.sql`INSERT INTO "Notification" ("id","userId","title","body") VALUES (${randomUUID()},${r.userId},'تم قبول طلب الشحن',${`تمت إضافة ${amount} ج.م إلى محفظتك. الرصيد الحالي ${newBalance} ج.م.`})`)
