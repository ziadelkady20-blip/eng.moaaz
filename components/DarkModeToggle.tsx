...
const icon = (dark: boolean) => dark
  ...
  : '<svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path d="M20.5 15.3A8.5 8.5 0 1 1 8.7 3.5 8.5 8.5 0 0 0 20.5 15.3Z"/></svg>'
...
export default function DarkModeToggle() {
  useEffect(() => {
    ...
    const install = () => {
      ...
      const existing = document.querySelector('.site-header button.theme-toggle') as HTMLButtonElement | null
      if (existing) {
        button = existing
      } else {
        const placeholder = document.querySelector('.site-header .theme-toggle') as HTMLElement | null
        if (!placeholder) return false

        const realButton = document.createElement('button')
        realButton.type = 'button'
        realButton.className = 'theme-toggle'
        realButton.setAttribute('data-theme-toggle', 'true')
        placeholder.replaceWith(realButton)
        button = realButton
      }
      ...
      const apply = (dark: boolean) => {
        ...
        if (button) {
          ...
          button.setAttribute('title', dark ? 'الوضع النهاري' : 'الوضع الليلي')
        }
      }
      ...
      cleanup = () => button?.removeEventListener('click', toggle)
      return true
    }
  ...
  return (
    <style jsx global>{`
      ...
      .site-header .theme-toggle svg{width:20px!important;height:20px!important;display:block!important;stroke:currentColor!important;stroke-width:2!important;stroke-linecap:round!important;stroke-linejoin:round!important}
      ...
      @media(max-width:680px){.site-header .theme-toggle{width:40px!important;min-width:40px!important;height:40px!important;flex-basis:40px!important}.site-header .theme-toggle svg{width:19px!important;height:19px!important}}
    `}</style>
  )
}
