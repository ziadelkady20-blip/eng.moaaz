...
function LoadingShell() {
  return (
    <main className="student-shell student-loading-shell" aria-busy="true" aria-label="جاري تحميل المنصة">
      <div className="student-header student-skeleton-header">
        <div className="student-header-inner">
          <div className="student-brand">
            <div className="student-skeleton student-skeleton-logo" />
            <div className="student-skeleton-copy"><div className="student-skeleton student-skeleton-name" /><div className="student-skeleton student-skeleton-sub" /></div>
          </div>
          <div className="student-skeleton-nav"><i /><i /><i /><i /></div>
          <div className="student-actions"><div className="student-skeleton student-skeleton-btn" /><div className="student-skeleton student-skeleton-btn" /><div className="student-skeleton student-skeleton-avatar" /></div>
        </div>
      </div>
      <div className="student-layout">
        <aside className="student-sidebar"><div className="student-sidebar-card student-skeleton-card"><div className="student-skeleton student-skeleton-title" />{Array.from({length:8}).map((_,i)=><div key={i} className="student-skeleton student-skeleton-row" />)}</div></aside>
        <section className="student-content"><div className="student-skeleton student-skeleton-heading" /><div className="student-skeleton student-skeleton-text" /><div className="student-skeleton-grid">{Array.from({length:4}).map((_,i)=><div key={i} className="student-skeleton student-skeleton-stat" />)}</div><div className="student-skeleton student-skeleton-panel" /><div className="student-skeleton-grid student-skeleton-grid-two"><div className="student-skeleton student-skeleton-panel small" /><div className="student-skeleton student-skeleton-panel small" /></div></section>
      </div>
      <div className="student-loading-label"><span className="student-spinner" />جاري تحميل حسابك...</div>
    </main>
  )
}
  ...
  return (
    <>
      <style>{`
        ...
        .student-header{position:relative;z-index:60;height:76px;background:rgba(255,255,255,.98);border-bottom:1px solid #eee8f4;box-shadow:0 7px 24px rgba(42,24,65,.07);backdrop-filter:blur(16px)}
        ...
        .student-layout{width:min(1360px,calc(100% - 32px));margin:auto;display:grid;grid-template-columns:250px minmax(0,1fr);gap:24px;padding:28px 0 0;direction:ltr}.student-sidebar{direction:rtl}.student-sidebar-card{position:relative;top:auto;background:#fff;border:1px solid #eee8f4;border-radius:22px;padding:12px;box-shadow:0 12px 30px rgba(42,24,65,.06)}.student-sidebar-title{padding:10px 12px 14px;border-bottom:1px solid #eee8f4;margin-bottom:8px;color:#6d2fa3;font-size:20px;font-weight:950}.student-sidebar a{display:flex;align-items:center;gap:10px;padding:11px 12px;border-radius:12px;color:#383043;font-size:13px;font-weight:850}.student-sidebar a:hover,.student-sidebar a.active{background:#f4ecfa;color:#6d2fa3}.student-content{direction:rtl;min-width:0}.student-loading{min-height:100vh;display:grid;place-items:center;background:#fbfaff;color:#6d2fa3;font-weight:900}
        .student-mobile-menu{display:none}
        .student-loading-shell{min-height:100vh;position:relative;overflow:hidden}.student-skeleton-header{opacity:1}.student-skeleton{position:relative;overflow:hidden;background:#eee9f5;border-radius:12px}.student-skeleton:after{content:"";position:absolute;inset:0;transform:translateX(-100%);background:linear-gradient(90deg,transparent,rgba(255,255,255,.8),transparent);animation:studentShimmer 1.25s ease-in-out infinite}.student-skeleton-logo{width:48px;height:48px;border-radius:13px}.student-skeleton-copy{display:grid;gap:6px}.student-skeleton-name{width:132px;height:12px}.student-skeleton-sub{width:90px;height:8px}.student-skeleton-nav{display:flex;justify-content:center;gap:8px}.student-skeleton-nav i{display:block;width:72px;height:34px;border-radius:12px;background:#eee9f5;position:relative;overflow:hidden}.student-skeleton-nav i:after{content:"";position:absolute;inset:0;transform:translateX(-100%);background:linear-gradient(90deg,transparent,rgba(255,255,255,.8),transparent);animation:studentShimmer 1.25s ease-in-out infinite}.student-skeleton-nav i:nth-child(2){animation-delay:.1s}.student-skeleton-nav i:nth-child(3){animation-delay:.2s}.student-skeleton-nav i:nth-child(4){animation-delay:.3s}.student-skeleton-btn{width:82px;height:40px}.student-skeleton-avatar{width:40px;height:40px;border-radius:50%}.student-skeleton-card{min-height:470px}.student-skeleton-title{width:120px;height:22px;margin:4px 10px 18px}.student-skeleton-row{height:38px;margin:7px 0}.student-skeleton-heading{width:min(310px,65%);height:34px}.student-skeleton-text{width:min(480px,80%);height:14px;margin-top:12px}.student-skeleton-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;margin-top:26px}.student-skeleton-stat{height:108px;border-radius:18px}.student-skeleton-panel{height:180px;border-radius:22px;margin-top:18px}.student-skeleton-grid-two{gap:18px}.student-skeleton-panel.small{margin-top:0;height:150px}.student-loading-label{position:fixed;left:50%;bottom:24px;transform:translateX(-50%);display:inline-flex;align-items:center;gap:9px;background:rgba(255,255,255,.96);border:1px solid #eee8f4;border-radius:999px;padding:10px 16px;box-shadow:0 10px 26px rgba(42,24,65,.09);color:#6d2fa3;font-size:12px;font-weight:900;z-index:100}.student-spinner{width:15px;height:15px;border:2px solid #eadcf4;border-top-color:#6d2fa3;border-radius:50%;animation:studentSpin .8s linear infinite}@keyframes studentShimmer{100%{transform:translateX(100%)}}@keyframes studentSpin{to{transform:rotate(360deg)}}
        @media(max-width:1050px){.student-header-inner{grid-template-columns:1fr auto}.student-top-nav{display:none}.student-actions{min-width:0}.student-menu-button{display:inline-flex;align-items:center;justify-content:center;width:42px;height:42px;border:1px solid #e7dfef;border-radius:12px;background:#fff;color:#6d2fa3}.student-layout{display:block;width:min(100%,calc(100% - 24px));padding-top:18px}.student-sidebar{display:none}.student-mobile-menu{position:fixed;top:88px;right:12px;left:12px;z-index:70;background:#fff;border:1px solid #eee8f4;border-radius:18px;box-shadow:0 18px 42px rgba(42,24,65,.14);padding:10px;display:flex;flex-direction:column;gap:3px;max-height:calc(100vh - 170px);overflow:auto}.student-mobile-menu a,.student-mobile-menu button{min-height:44px;padding:0 12px;border-radius:11px;display:flex;align-items:center;gap:10px;border:0;background:transparent;font:inherit;font-size:13px;font-weight:900;color:#383043;text-align:right}.student-mobile-menu a.active,.student-mobile-menu a:hover{background:#f4ecfa;color:#6d2fa3}.student-mobile-menu button{color:#b42318}.student-mobile-menu .mobile-divider{height:1px;background:#eee8f4;margin:5px 0}.student-mobile-bar{position:fixed;bottom:12px;left:12px;right:12px;z-index:80;background:rgba(255,255,255,.97);border:1px solid #eee8f4;border-radius:18px;box-shadow:0 12px 30px rgba(42,24,65,.13);display:grid;grid-template-columns:repeat(4,1fr);padding:7px}.student-mobile-bar a{display:grid;place-items:center;gap:4px;padding:7px 3px;border-radius:12px;font-size:10px;font-weight:900;color:#5e5668}.student-mobile-bar a.active{background:#f3eafa;color:#6d2fa3}.student-shell{padding-bottom:98px}.student-skeleton-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
        @media(max-width:560px){.student-header{height:66px}.student-header-inner{width:calc(100% - 14px);gap:6px}.student-brand img{width:40px;height:40px}.student-brand strong{font-size:14px}.student-brand span{display:none}.student-actions{gap:5px}.student-action{width:40px;height:40px;padding:0;justify-content:center}.student-action span{display:none}.student-avatar{width:38px;height:38px;flex-basis:38px}.student-layout{width:calc(100% - 14px)}.student-mobile-menu{top:75px;right:7px;left:7px}.student-skeleton-nav{display:none}.student-skeleton-btn{width:40px}.student-skeleton-grid{gap:9px}.student-skeleton-stat{height:90px}.student-skeleton-panel.small{height:120px}.student-loading-label{bottom:92px;font-size:11px;max-width:calc(100% - 24px);white-space:nowrap}}
      `}</style>
      ...
      <main className="student-shell">
        ...
        {open && <>
          ...
          <div className="student-mobile-menu">
            ...
            <Link href="/" onClick={() => setOpen(false)}><Home size={17}/> الصفحة الرئيسية</Link>
            <Link href="/student/profile" onClick={() => setOpen(false)}><User size={17}/> {user?.name || 'الملف الشخصي'}</Link>
            <Link href={roleTarget(user?.role)} onClick={() => setOpen(false)}><LayoutDashboard size={17}/> لوحة الحساب</Link>
